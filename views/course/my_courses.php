...
<div class="container">
    ...
    <div class="filter-section">
        <form id="courseFilterForm" method="GET" action="/study_sharing/course/myCourses">
    ...
    <?php if (empty($courses)): ?>
        <div class="empty-state">
            ...
            <p class="text-muted">Bạn chưa tham gia khóa học nào hoặc không tìm thấy kết quả phù hợp.</p>
            <?php if (!isset($_SESSION['account_id'])): ?>
                <button class="btn btn-primary mt-3" data-bs-toggle="modal" data-bs-target="#loginModal" style="padding: 0.4rem 0.8rem; font-size: 0.875rem;">
                    <i class="bi bi-box-arrow-in-right" style="font-size: 0.8rem; margin-right: 4px;"></i> Đăng nhập để tham gia khóa học
                </button>
            <?php else: ?>
                <a href="/study_sharing/course/list" class="btn btn-primary mt-3" style="padding: 0.4rem 0.8rem; font-size: 0.875rem;">
                    <i class="bi bi-search" style="font-size: 0.8rem; margin-right: 4px;"></i> Tìm khóa học mới
                </a>
            <?php endif; ?>
        </div>
    <?php else: ?>
        ...
        <nav aria-label="Page navigation" class="mt-5">
            <ul class="pagination justify-content-center">
                <?php if ($page > 1): ?>
                    <li class="page-item">
                        <a class="page-link" href="?page=<?php echo $page - 1; ?>&query=<?php echo urlencode($query); ?>">
                ...
                if ($start > 1): ?>
                    <li class="page-item"><a class="page-link" href="?page=1&query=<?php echo urlencode($query); ?>">1</a></li>
                    <?php if ($start > 2): ?>
                ...
                <?php if ($end < $totalPages): ?>
                    <?php if ($end < $totalPages - 1): ?>
                        <li class="page-item disabled"><span class="page-link">...</span></li>
                    <?php endif; ?>
                    <li class="page-item"><a class="page-link" href="?page=<?php echo $totalPages; ?>&query=<?php echo urlencode($query); ?>"><?php echo $totalPages; ?></a></li>
                <?php endif; ?>
