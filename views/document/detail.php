    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div id="comments-container" class="mt-5" data-is-logged-in="<?php echo isset($_SESSION['account_id']) ? 'true' : 'false'; ?>" data-current-user-id="<?php echo isset($_SESSION['account_id']) ? $_SESSION['account_id'] : 0; ?>">
                ...
                <?php else: ?>
                    <?php
                    function renderReplies($replies, $document, $pdo, $level = 1)
                    {
                        ...
                        foreach ($replies as $reply) {
                            ...
                            // Lọc nội dung bình luận để chỉ cho phép thẻ <span>
                            $safeCommentText = strip_tags($reply['comment_text'], '<span>');
                            ...
                            $html .= '
                                ...
                                <p class="mb-0">' . $safeCommentText . '</p>
                            ...
                            if (!empty($reply['replies'])) {
                                $html .= renderReplies($reply['replies'], $document, $pdo, $level + 1);
                            }
                            $html .= '</div>';
                        }
                        $html .= '</div>';
                    ...
                    <?php foreach ($comments as $comment): ?>
                        <div class="comment-item" data-comment-id="<?php echo $comment['comment_id']; ?>">
                            <?php
                            $commentUser = isset($comment['user']) && is_array($comment['user']) ? $comment['user'] : ['avatar' => null, 'full_name' => 'Ẩn danh'];
                            // Lọc nội dung bình luận để chỉ cho phép thẻ <span>
                            $safeCommentText = strip_tags($comment['comment_text'], '<span>');
                            ?>
                            <div class="d-flex align-items-center mb-2 position-relative">
                            ...
                            <p class="mb-0"><?php echo $safeCommentText; ?></p>
                            ...
                            <?php if (!empty($comment['replies'])): ?>
                                <?php echo renderReplies($comment['replies'], $document, $this->db); ?>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
