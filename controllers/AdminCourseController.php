...
class AdminCourseController
{
    ...
    public function admin_add()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!isset($_SESSION['account_id']) || !isset($_SESSION['role']) || $_SESSION['role'] !== 'admin') {
                $_SESSION['message'] = 'Bạn không có quyền thêm khóa học!';
                $_SESSION['message_type'] = 'danger';
            ...
            if (empty($course_name) || $creator_id <= 0 || $max_members <= 0) {
                $_SESSION['message'] = 'Tên khóa học, người tạo và số lượng thành viên tối đa là bắt buộc!';
                $_SESSION['message_type'] = 'danger';
                header('Location: /study_sharing/AdminCourse/manage');
                exit;
            }
            ...
            try {
                // Sử dụng model để thêm khóa học (chỉ hỗ trợ course_name, description, creator_id)
                $result = $this->courseModel->createCourse($course_name, $description, $creator_id);

                if ($result) {
                    // Lấy course_id vừa tạo
                    $course_id = $this->pdo->lastInsertId();

                    // Cập nhật các trường bổ sung không có trong model
                    ...
                    $_SESSION['message'] = 'Thêm khóa học thất bại!';
                    $_SESSION['message_type'] = 'danger';
                }
                header('Location: /study_sharing/AdminCourse/manage');
            } catch (PDOException $e) {
    ...
    public function admin_delete()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                // Kiểm tra số thành viên và tài liệu liên quan
                $counts = [
                    'member_count' => 0,
                    'document_count' => 0
                ];
                ...
                // Xóa khóa học (model không có phương thức delete, nên dùng truy vấn trực tiếp)
